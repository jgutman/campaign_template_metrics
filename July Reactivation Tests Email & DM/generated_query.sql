WITH campaign_lists AS (
    ...
    FROM analytics.july_reactivation_email_dm a
    LEFT JOIN dw.marketing_offers md
    ON md.internal_user_id = a.user_id
    AND ((md.offer_campaign_name = 'July Reactivation 25 offer'
        AND md.discount_name = '$25 off next box'
        AND a.target_name = '25 Off EMAIL ONLY - LAPSED Test 1')
        OR (md.offer_campaign_name = 'July Reactivation 25 offer'
        AND md.discount_name = '$25 off next box'
        AND a.target_name = '25 Off EMAIL and DM Postcard - LAPSED Test 2')
        OR (md.offer_campaign_name = 'July Reactivation 25 offer'
        AND md.discount_name = '$25 off next box'
        AND a.target_name = '25 Off EMAIL and DM Trifold - LAPSED Test 3')
        OR (md.offer_campaign_name = 'July Reactivation 50 offer'
        AND md.discount_name = '$10 off next 5 boxes'
        AND a.target_name = '50 Off EMAIL ONLY - LAPSED Test 4')
        OR (md.offer_campaign_name = 'July Reactivation 50 offer'
        AND md.discount_name = '$10 off next 5 boxes'
        AND a.target_name = '50 Off EMAIL and DM Postcard - LAPSED Test 5')
        )
    CROSS JOIN (
    SELECT 'promo_period' AS promo_period,
        '2017-09-01' AS end_date
	UNION ALL
    ...
    INNER JOIN dw.user_subscription_events use
        ...
        subscription_changed_at)) >= '2017-07-20'
        AND DATE(convert_timezone('America/New_York',
